...
class MatchRepository(private val baseUrl: String) : IMatchRepository {
    ...
    override suspend fun create(match: Match): String = suspendTransaction {
        val dao = MatchDAO.new {
            ...
            saves = match.saves; blockedShots = match.blockedShots
            fouls = match.fouls; yellowCards = match.yellowCards
            redCards = match.redCards; offsides = match.offsides
            possession = match.possession; passAccuracy = match.passAccuracy
            totalPasses = match.totalPasses; corners = match.corners
            dribbles = match.dribbles; aerialDuelsWon = match.aerialDuelsWon
            rebounds = match.rebounds; steals = match.steals
            blocks = match.blocks; turnovers = match.turnovers
            threePointers = match.threePointers; notes = match.notes
    ...
    override suspend fun update(userId: String, matchId: String, newMatch: Match): Boolean = suspendTransaction {
        ...
        dao.result = newMatch.result; dao.goals = newMatch.goals
        dao.assists = newMatch.assists; dao.shots = newMatch.shots
        dao.shotsOnTarget = newMatch.shotsOnTarget; dao.bigChances = newMatch.bigChances
        dao.bigChancesMissed = newMatch.bigChancesMissed; dao.tackles = newMatch.tackles
        dao.interceptions = newMatch.interceptions; dao.clearances = newMatch.clearances
        dao.saves = newMatch.saves; dao.blockedShots = newMatch.blockedShots
        dao.fouls = newMatch.fouls; dao.yellowCards = newMatch.yellowCards
        dao.redCards = newMatch.redCards; dao.offsides = newMatch.offsides
        dao.possession = newMatch.possession; dao.passAccuracy = newMatch.passAccuracy
        dao.totalPasses = newMatch.totalPasses; dao.corners = newMatch.corners
        dao.dribbles = newMatch.dribbles; dao.aerialDuelsWon = newMatch.aerialDuelsWon
        dao.rebounds = newMatch.rebounds; dao.steals = newMatch.steals
        dao.blocks = newMatch.blocks; dao.turnovers = newMatch.turnovers
        dao.threePointers = newMatch.threePointers; dao.notes = newMatch.notes
